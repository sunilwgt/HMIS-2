...
import { DoctorOptions, DischargeTypeOption, fileupload } from '../../models/opd';
...
export class DocumentComponent extends BaseComponent implements OnInit {
  ...
  private fileuploadModel: fileupload = new fileupload();
...
private patientprn;
private patientprnn;
  constructor(baseservice: BaseServices, private helperFunc: HelperFunction, private renderer: Renderer2, public datepipe: DatePipe, private externalApis: HmisExternalApisService) {
    super(baseservice)
    this.showNav[0] = true;
  ...
  hmisApiSubscribe(data: any): void {
    ...
    if (data.resulttype === RESULT_TYPE_GET_SELECTED_PATIENT_BY_ID) {
      ...
      this.patientprn = data.result[0].patient_registration_no
    ...
    if (data.resulttype === RESULT_TYPE_GET_ALL_DOCTOR_LIST) {
      ...
      console.log('doclist', this.doctorListOption);
    }
    ...
    if (data.resulttype === RESULT_TYPE_POST_FILE) {
  ...
  private getFiles(evnt: any): void {
    ...
    this.files = evnt.srcElement.files;
  ...
  submitClickHandler() {
    this.fileuploadModel.Reffered_Doctor = this.compData.Reffered_Doctor;
    this.fileuploadModel.description = this.compData.description;
    this.fileuploadModel.file = this.files[0];
    this.fileuploadModel.pateint_reg_no = this.patientprn;
    // this.externalApis.loadProfileImage(this.files[0]);
    console.log('model' , this.fileuploadModel);
    this.hmisApi.uploadfile(this.fileuploadModel);
