...
@Component({
  selector: 'assignpermission-list',
  templateUrl: './assignpermission-list.component.html',
  styleUrls: ['./assignpermission-list.component.scss'],
  providers: [
    { provide: 'Window', useValue: window }
  ],
  encapsulation: ViewEncapsulation.None
})
export class AssignPermissionListComponent extends BaseComponent implements OnInit {
  ...
  private permissionsResource = new DataTableResource([]);
  private permissions = [];
  private arrangepermissionsdata = [];
  ...
  hmisApiSubscribe(data: any): void {
    if (data.resulttype === RESULT_TYPE_GET_ALL_PERMISSIONS) {
      this.permissions = data.result;
      console.log('all permissions data' , data);
    
      this.arrangepermissionsdata = this.permissions;
      this.permissionsResource = new DataTableResource(this.arrangepermissionsdata);
      this.permissionsResource.count().then(count => {
        this.permissionsCount = count;
      });
      const para = { offset: 0, limit: 15 }
      this.reloadPermissions(para);
    }
  ...
  reloadPermissions(params) {
    this.permissionsResource.query(params).then(permissions => this.permissions = permissions);
  }
