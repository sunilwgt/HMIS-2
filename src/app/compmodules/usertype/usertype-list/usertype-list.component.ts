...
export class UsertypeListComponent extends BaseComponent implements OnInit {
  ...
  private usertypeResource = new DataTableResource([]);
  private usertype = [];
  private arrrangedusertypedata = [];
  private usertypecount = 0;
  ...
  hmisApiSubscribe(data: any): void {
    if (data.resulttype === RESULT_TYPE_GET_ALL_USERSTYPE) {
      console.log('data' , data);
      this.usertype = data.result;
     
      this.arrrangedusertypedata = this.usertype;
      this.usertypeResource = new DataTableResource(this.arrrangedusertypedata);
      this.usertypeResource.count().then(count => {
        this.usertypecount = count;
      });
      const para = { offset: 0, limit: 15 }
      this.reloadusertype(para);
    }

    if (data.resulttype === RESULT_TYPE_DELETE_USER_TYPE) {
  ...
  reloadusertype(params) {
    this.usertypeResource.query(params).then(usertype => this.usertype = usertype);
  }
  ...
  private clickEventHandler(eventObj: ActionType, mode, item): void {
    ...
    switch (mode) {
      case MODE_EDIT:
        this.compLoadManager.redirect(RL_USERTYPE);
      ...
      case MODE_DELETE:
        this.hmisApi.deleteUSerTypeAsPerId(item.role_id);
