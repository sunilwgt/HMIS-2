...
import { ForgotPassword, User } from '../models/user';
...
// const PATIENT_URL: string = "https://192.168.1.166:443/api/Patient";
// const HMIS_URL: string = "https://192.168.1.166:443/api/";
// const ALTERNATE_URL: string = "192.168.1.166:443/";

const PATIENT_URL: string = "https://hmismultitenantwebapi.azurewebsites.net/api/Patient";
const HMIS_URL: string = "https://hmismultitenantwebapi.azurewebsites.net/api/";
const ALTERNATE_URL: string = "https://hmismultitenantwebapi.azurewebsites.net/";
const uploadurl:string = "https://hmismultitenantwebapi.azurewebsites.net/api/File/UploadToAzureFileStorage?containerName=4F4436FD-4BCB-412A-AE30-60E917C768DB";
...
const REQUEST_POST_FILE: string = "POST";
 const REQUEST_GET_FILE:string = "GET"
...
export class HmisApisService {
  ...
  public _hmistoken: string;
  private _userDetail: UserDetail;

  constructor(private hmisHttp: HttpClient, private loader: LoaderService , private http:Http) { }
  ...
  public getAdmittedPatientList(from , to , searchKey: string, loader: boolean = true): void {

    let urlstr: string = `${HMIS_URL}PatientAdmission/GetPagedPatientAdmissionData${this.createSearchStructureForAdmiitedPatient(searchKey ,from , to)}`;
    this.genericApiCall(urlstr, RESULT_TYPE_GET_ADMITTED_PATIENT_LIST, null, REQUEST_GET, loader);
  }

  private createSearchStructureForAdmiitedPatient(searchstr: string, fromdate , todate , pageno = 0, _pagesize = 10, pageSize = 100, sortingOrder = "LastName_DSC", rangeSearch = ""): string {
    return `?&pageNumber=${pageno}&pageSize=${pageSize}&sortingOrder=${sortingOrder}&querySearch=${searchstr}&fromdate=${fromdate}&todate=${todate}`;
  }
...
public getrejectedListforchart( searchKey: string, loader: boolean = true): void {
  let urlstr: string = `${HMIS_URL}PatientApproval/GetApproverDashBoardRejectedWorkflowItems?assignedTo=${this._userDetail.uid}`; //&${this.ccc(searchKey)}
  this.genericApiCall(urlstr, RESULT_TYPE_GET_REJECTED_LIST_FOR_CHART, null, REQUEST_GET, loader);
}
public getApprovedListforchart( searchKey: string, loader: boolean = true): void {
  ...
  this.genericApiCall(urlstr, RESULT_TYPE_GET_APPROVED_LIST_FOR_CHART, null, REQUEST_GET, loader);
}
public getPendingforchart( searchKey: string, loader: boolean = true): void {
  ...
  this.genericApiCall(urlstr, RESULT_TYPE_GET_PENDING_LIST_FOR_CHART, null, REQUEST_GET, loader);
}
  ...
  public getrejectedList( searchKey: string, loader: boolean = true): void {
    let urlstr: string = `${HMIS_URL}PatientApproval/GetApproverDashBoardRejectedWorkflowItems?assignedTo=${this._userDetail.uid}`; //&${this.ccc(searchKey)}
    this.genericApiCall(urlstr, RESULT_TYPE_GET_REJECTED_LIST, null, REQUEST_GET, loader);
  }

 

  private createSearchStructureForRejectedList(searchstr: string, pageno = 0, _pagesize = 1, pageSize = 30, sortingOrder = "FatherName_ASC", rangeSearch = ""): string {
    return `?&pageNumber=${pageno}&_pageSize=${_pagesize}&pageSize=${pageSize}&querySearch=${searchstr}&sortingOrder=${sortingOrder}&rangeSearch=${rangeSearch}`;
  }
 ...
 //ApprovalStatus/GetPagedApprovalStatusData?&pageNumber=0&_pageSize=1&pageSize=50&sortingOrder=ApprovalStatusName_ASC&querySearch=
  ...
  public getdcertificatedatewise(from , to , searchKey: string, loader: boolean = true): void {
    let urlstr: string = `${HMIS_URL}PatientDischarge/GetPagedDischargeData${this.createSearchStructurefordcertificatedatewise(from , to ,searchKey)}`;
    this.genericApiCall(urlstr, RESULT_TYPE_GET_D_CERTIFICATE_DATEWISE, null, REQUEST_GET, loader);
  }
  private createSearchStructurefordcertificatedatewise(f , t ,searchstr: string, pageno = 0, pagesize = 10,sortingOrder = ""): string {
    return `?&pageNumber=${pageno}&pageSize=${pagesize}&sortingOrder=${sortingOrder}&querySearch=${searchstr}&fromdate=${f}&todate=${t}`;
  }
  ...
  public getHospitalSettingsLIST(searchKey: string, loader: boolean = true): void {
    let urlstr: string = `${HMIS_URL}HospitalDetails/GetPagedHospitalDetailsData${this.createSearchStructureForHospitallist(searchKey)}`;
    this.genericApiCall(urlstr, RESULT_TYPE_GET_HOSPITAL_DETAIL_LIST_LIST, null, REQUEST_GET, loader);
  }

  private createSearchStructureForHospitallist(searchstr: string, pageno = 0, _pagesize = 1, pageSize = 15, sortingOrder = "HospitalName_Dsc", rangeSearch = ""): string {
    return `?&pageNumber=${pageno}&_pageSize=${_pagesize}&pageSize=${pageSize}&querySearch=${searchstr}&sortingOrder=${sortingOrder}&rangeSearch=${rangeSearch}`;
  }
  ...
  public patientSearch(from , to , searchKey: string, loader: boolean = true): void {
    let urlstr: string = `${PATIENT_URL}/GetPagedPatientDetailsList${this.createSearchStructureforreg(from , to ,searchKey)}`;
    this.genericApiCall(urlstr, RESULT_TYPE_GET_PATIENT_SEARCH, null, REQUEST_GET, loader);
  }
  private createSearchStructureforreg(f , t ,searchstr: string, pageno = 0, pagesize = 10): string {
    return `?&pageNumber=${pageno}&pageSize=${pagesize}&sortingOrder=&querySearch=${searchstr}&fromdate=${f}&todate=${t}`;
  }

  // https://localhost:44391/api/Patient/GetPagedPatientDetailsList?
  // pageNumber=0&pageSize=10&sortingOrder=&querySearch=&fromdate=12-Jun-2017&todate=12-Jun-2019
  ...
  public getotlistdatewise(from , to , searchKey: string, loader: boolean = true): void {

    let urlstr: string = `${HMIS_URL}OperationTheatre/GetPagedOTData${this.createSearchStructureForotdatewise(searchKey ,from , to)}`;
    this.genericApiCall(urlstr, RESULT_TYPE_GET_OT_LIST_DATEWISE, null, REQUEST_GET, loader);
  }

  private createSearchStructureForotdatewise(searchstr: string, fromdate , todate , pageno = 0, _pagesize = 1, pageSize = 1000, sortingOrder = "FatherName_ASC", rangeSearch = ""): string {
    return `?&pageNumber=${pageno}&pageSize=${pageSize}&sortingOrder=${sortingOrder}&querySearch=${searchstr}&fromdate=${fromdate}&todate=${todate}`;
  }
  ...
  public getPatientListForNewbornWithAdmissionSequence(searchKey: string, loader: boolean = true): void {
    let urlstr: string = `${HMIS_URL}patientadmission/GetAutocompletePatientAdmissionForOperation?searchNumber=${searchKey}`;
    this.genericApiCall(urlstr, RESULT_TYPE_GET_OPERATION_THEATRE_BY_ADMISSION_SEQ_FOR_NEWBORN, null, REQUEST_GET, loader);
  }
  ...
  public getPatientDetailsForNewbornOnSearchId(searchKey: string, loader: boolean = true): void {
    let urlstr: string = `${HMIS_URL}PatientAdmission/GetPatientAdmissionDetailsForOT?&admissionID=${searchKey}`;
    this.genericApiCall(urlstr, RESULT_TYPE_GET_PATIENT_DETAILS_FOR_NEWBORN, null, REQUEST_GET, loader);
  }
  ...
  public getnewborndatewise(from , to , searchKey: string, loader: boolean = true): void {
    let urlstr: string = `${HMIS_URL}newborn/GetPagedPatientData${this.createSearchStructurefornewbordatewise(from , to ,searchKey)}`;
    this.genericApiCall(urlstr, RESULT_TYPE_GET_NEW_BORN_LIST_DATEWISE, null, REQUEST_GET, loader);
  }
  private createSearchStructurefornewbordatewise(f , t ,searchstr: string, pageno = 0, pagesize = 10,sortingOrder = "FatherName_ASC"): string {
    return `?&pageNumber=${pageno}&pageSize=${pagesize}&sortingOrder=${sortingOrder}&querySearch=${searchstr}&fromdate=${f}&todate=${t}`;
  }


  public setLiscence(obj) {
    let urlstr: string = `${HMIS_URL}LicenseReminder`;
    this.genericApiCall(urlstr, RESULT_TYPE_SET_LISCENCE, obj, REQUEST_POST);
  }

  public getLiscenceSearch(searchKey: string, loader: boolean = true): void {
    let urlstr: string = `${HMIS_URL}LicenseReminder/GetPagedLicenseReminderData${this.createSearchStructureForLiscence(searchKey)}`;
    this.genericApiCall(urlstr, RESULT_TYPE_GET_LISCENCE_LIST, null, REQUEST_GET, loader);
  }

  
  private createSearchStructureForLiscence(searchstr: string, pageno = 0, _pagesize = 1, pageSize = 50, sortingOrder = "LicenseReminderName"): string {
    return `?&pageNumber=${pageno}&_pageSize=${_pagesize}&pageSize=${pageSize}&sortingOrder=${sortingOrder}&querySearch=${searchstr}`;
  }
  public deleteLiscence(lId: string) {
    let urlstr: string = `${HMIS_URL}LicenseReminder?licensereminderId=${lId}&deletedBy=${this._userDetail.uid}`;
    this.genericApiCall(urlstr, RESULT_TYPE_DELETE_LISCENCE, "", REQUEST_DELETE);
  }
  public setLiscenceAsPerId(lid: any, lObj:any) {
    let urlstr: string = `${HMIS_URL}LicenseReminder?id=${lid}`;
    this.genericApiCall(urlstr, RESULT_TYPE_EDIT_LISCENCE, lObj, REQUEST_PUT);
  }
  ...
  public uploadfile(extObj): void {
    const  urlstr: string = uploadurl;
    this.genericApiCall(urlstr, RESULT_TYPE_POST_FILE, extObj, REQUEST_POST_FILE);
  }

  public getUploadFile(loader: boolean = true) {
    let urlstr: string = getfileurl;
    this.genericApiCall(urlstr, RESULT_TYPE_GET_FILES, null, REQUEST_GET_FILE, loader);
  }

  // users management


  public userSearch(searchKey: string, loader: boolean = true): void {
    let urlstr: string = `${HMIS_URL}/Admin/Get`;
    this.genericApiCall(urlstr, RESULT_TYPE_GET_ALL_APPUSERS, null, REQUEST_GET, loader);
  }

 

  public userTypeSearch(searchKey: string, loader: boolean = true): void {
    let urlstr: string = `${HMIS_URL}Roles/GetPagedRoleData?pageNumber=0&_pageSize=1&pageSize=50&sortingOrder=ApprovalStatusName_ASC&querySearch`;
    this.genericApiCall(urlstr, RESULT_TYPE_GET_ALL_USERSTYPE, null, REQUEST_GET, loader);
  }
  public userTypeSearchValuePair(searchKey: string, loader: boolean = true): void {
    let urlstr: string = `${HMIS_URL}Roles/GetActiveNameValuePairRoles`;
    this.genericApiCall(urlstr, RESULT_TYPE_GET_ALL_USERSTYPE_VAlUE_PAIR, null, REQUEST_GET, loader);
  }
  
  public userTypeSearchPermissionValuePair(searchKey: string, loader: boolean = true): void {
    let urlstr: string = `${HMIS_URL}Permissions/GetActiveNameValuePairPermnission`;
    this.genericApiCall(urlstr, RESULT_TYPE_GET_USER_TYPE_PERMISSION_VAlUE_PAIR, null, REQUEST_GET, loader);
  }
  public setUser(userObj: User) {
    let urlstr: string = `${HMIS_URL}Admin`;
    this.genericApiCall(urlstr, RESULT_TYPE_ADD_USER, userObj, REQUEST_POST);
  }

public setUserType(userTypeObj: UserType) {
    let urlstr: string = `${HMIS_URL}Roles`;
    this.genericApiCall(urlstr, RESULT_TYPE_ADD_USER_TYPE, userTypeObj, REQUEST_POST);
  }
  public deleteUSerTypeAsPerId(deletedById: string) {
    let urlstr: string = `${HMIS_URL}?Id=${deletedById}&deletedBy=${this._userDetail.uid}`;
    this.genericApiCall(urlstr, RESULT_TYPE_DELETE_USER_TYPE, "", REQUEST_DELETE);
  }

  public PermissionSearch(searchKey: string, loader: boolean = true): void {
    let urlstr: string = `${HMIS_URL}Permissions/GetPagedPermissionsData?pageNumber=0&_pageSize=1&pageSize=50&sortingOrder=ApprovalStatusName_ASC&querySearch`;
    this.genericApiCall(urlstr, RESULT_TYPE_GET_ALL_PERMISSIONS, null, REQUEST_GET, loader);
  }

  public setPermission(userObj: Permissions) {
    let urlstr: string = `${HMIS_URL}Permissions`;
    this.genericApiCall(urlstr, RESULT_TYPE_ADD_PERMISSIONS, userObj, REQUEST_POST);
  }
  public getpermissionsbyroleid(roleid: string, loader: boolean = true): void {
    let urlstr: string = `${HMIS_URL}rolepermission/GetAllPermissionAssignedToRolesByRole?roleId=${roleid}`;
    this.genericApiCall(urlstr, RESULT_TYPE_GET_PERMISSIONS_BY_ROLE_ID, null, REQUEST_GET, loader);
  }
  public setpermissiontorole(permissionObj:any) {
    let urlstr: string = `${HMIS_URL}RolePermission/`;
    this.genericApiCall(urlstr, RESULT_TYPE_SET_PERMISSION_TO_ROLE, permissionObj, REQUEST_POST);
  }
  public removepermissiontorole(deleteobj: any) {
    let urlstr = `${HMIS_URL}RolePermission/?permissionid=${deleteobj.permission_id}&roleid=${deleteobj.role_id}`;
    this.genericApiCall(urlstr, RESULT_TYPE_DELETE_PERMISSION_FROM_ROLE, "", REQUEST_DELETE);
  }
  // public getallrolesbyuserid(userid: string, loader: boolean = true): void {
  //   let urlstr: string = `${HMIS_URL}rolepermission/GetAllPermissionAssignedToRoles?userId=${userid}`;
  //   this.genericApiCall(urlstr, RESULT_TYPE_GET_ALL_ROLES_BY_USERID, null, REQUEST_GET, loader);
  // }
  
  public getrolesofauserbyid(userid){
    const httpo = {
      headers: new HttpHeaders().set("Token", this.hmistoken).set("Tenant_Key", "MedicareDb")
    };
    return this.hmisHttp.get(`${HMIS_URL}rolepermission/GetAllPermissionAssignedToRoles?userId=${userid}` , httpo)
  }
  
  public setroletouser(roleObj:any) {
    console.log('roleobj' , roleObj)
    let urlstr: string = `${HMIS_URL}UserRoles`;
    this.genericApiCall(urlstr, RESULT_TYPE_SET_ROLE_TO_USER, roleObj, REQUEST_POST);
  }

  public removeroletouser(deleteobj: any) {
    let urlstr = `${HMIS_URL}UserRoles?userid=${deleteobj.user_id}&roleid=${deleteobj.role_id}`;
    this.genericApiCall(urlstr, RESULT_TYPE_DELETE_ROLE_FROM_USER, "", REQUEST_DELETE);
  }
  //end api
  // downloadFile(m) {
  //   const httpOptFile  =  { headers: new HttpHeaders().set('Content-Type', 'multipart/form-data')
  //   .set("Token", this.hmistoken)
  ...
  //   return this.http
  //     .get(m, {responseType: ResponseContentType.Blob,})
  //     .map(res => {
  //       return {
...
const httpOptFile  =  { headers: new HttpHeaders().set('Content-Type', 'multipart/form-data')
    ...
    switch (requesttype){
      case REQUEST_GET:
        ...
        this.hmisHttp.get(url, httpOptFile).subscribe(
          data => {
            // console.log('(requestget  data)' ,data);
            if (activeLoader) this.loader.showLoader = false;
            this._result.next({ resulttype: rtype, result: data });
      ...
      case REQUEST_POST:
        ...
        this.hmisHttp.post(url, bodydata, httpOptFile).subscribe(
          data => {
            // console.log('(post data)' ,data);
            if (activeLoader) this.loader.showLoader = false;
            this._result.next({ resulttype: rtype, result: data });
